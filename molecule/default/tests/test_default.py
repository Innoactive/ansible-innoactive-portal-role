...
def test_docker_python_package_installed(host):
    pip_package_docker = host.pip_package.get_packages(pip_path="pip3").get("docker")
...
def test_services_are_running(host):
    ...
    with host.sudo():
        ...
        actual_service_names = [
            container.name for container in host.docker.get_containers(status="running")
        ]
